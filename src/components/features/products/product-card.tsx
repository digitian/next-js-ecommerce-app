...
export default function ProductCard({ product, layout = "grid" }: ProductCardProps) {
  ...
  return (
    <Link href={`/products/${product.slug}`} className="group block h-full">
      <Card className={`pt-0 h-full overflow-hidden border-transparent bg-transparent transition-colors hover:bg-muted/40 relative ${layout === 'list' ? 'flex flex-row p-0' : ''}`}>
        {/* Showcase Image */}
        <div className={`relative overflow-hidden bg-muted/20 ${layout === 'list' ? 'w-[300px] shrink-0' : ''}`}>
          <Image
            src={product.images[0]?.url}
            alt={product.images[0]?.alt || product.name}
            width={500}
            height={500}
            className={`object-cover transition-transform duration-500 group-hover:scale-105 w-full ${layout === 'list' ? 'h-full aspect-video' : 'h-[300px]'}`}
        ...
        {/* Content */}
        <CardContent className={`p-4 pt-4 flex flex-col justify-center ${layout === 'list' ? 'flex-1' : ''}`}>
          <h3 className="font-medium text-foreground transition-colors duration-200 group-hover:text-primary line-clamp-1">
            {product.name}
          </h3>
          <p className={`mt-1 text-muted-foreground ${layout === 'list' ? 'text-base line-clamp-3' : 'text-sm line-clamp-1'}`}>
